...
class SheetJSApp extends React.Component {
    ...
    createTemplate(){
        const wsTemlpate = {
            ...
            "!merges": [{"s": {"c": 1, "r": 0}, "e": {"c": 2, "r": 0}}, {
                "s": {"c": 3, "r": 0},
                "e": {"c": 4, "r": 0}
            }, {"s": {"c": 5, "r": 0}, "e": {"c": 8, "r": 0}}, {"s": {"c": 0, "r": 0}, "e": {"c": 0, "r": 1}}]
        };
        const data = [
            ["文件名称", "作品信息", null, "创作信息", null, "发表信息"],
            [null, "作品名称", "作品类型（ 文字/图片/视频）", "创作完成地点", "创作完成时间（xxxx-xx-xx）", "发表状态（已发表/未发表）", "首次发表日期(xxxx-xx-xx)", "首次发表地点", "首次发表网址"]
        ];
        const ws = XLSX.utils.aoa_to_sheet(data);
        Object.assign(ws, wsTemlpate);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "SheetJS");
        XLSX.writeFile(wb, "template"+new Date().getTime()+".xlsx");
    }
    render() { return (
        <DragDropFile handleFile={this.handleFile}>
            ...
            <div className="row"><div className="col-xs-12">
                ...
                <button className="btn btn-success" onClick={this.createTemplate}>ExportTemplate</button>
            </div></div>
            <div className="row"><div className="col-xs-12">
